package leetCode;

import java.util.ArrayList;
import java.util.Arrays;
import java.util.HashMap;
import java.util.List;
import java.util.Map;

public class Problem_1 {

	// 1. Two Sum
	/*
	 * Given an array of integers nums and an integer target, return indices of the
	 * two numbers such that they add up to target.
	 * 
	 * You may assume that each input would have exactly one solution, and you may
	 * not use the same element twice.
	 * 
	 * You can return the answer in any order.
	 * 
	 * 
	 * 
	 * Example 1:
	 * 
	 * Input: nums = [2,7,11,15], target = 9 Output: [0,1] Explanation: Because
	 * nums[0] + nums[1] == 9, we return [0, 1]. Example 2:
	 * 
	 * Input: nums = [3,2,4], target = 6 Output: [1,2] Example 3:
	 * 
	 * Input: nums = [3,3], target = 6 Output: [0,1]
	 * 
	 * 
	 * Constraints:
	 * 
	 * 2 <= nums.length <= 104 -109 <= nums[i] <= 109 -109 <= target <= 109 Only one
	 * valid answer exists.
	 * 
	 * 
	 * Follow-up: Can you come up with an algorithm that is less than O(n2) time
	 * complexity?
	 */

	public static void main(String args[]) {

	}

	/*
	 * public int[] twoSum(int[] nums, int target) { int[] result = new int[2];
	 * for(int i = 0; i < nums.length; i++) { for(int j = 1; j < nums.length; j++) {
	 * if(nums[i] + nums[j] == target && i != j) { result[0] = i; result[1] = j;
	 * return result; } } } return result; }
	 */

	public int[] twoSum(int[] nums, int target) {
		// Create a HashMap to store the number and its index
		Map<Integer, Integer> numberIndexMap = new HashMap<>();

		// Iterate through the array
		for (int currentIndex = 0; currentIndex < nums.length; currentIndex++) {
			int currentNumber = nums[currentIndex];

			// Calculate the number needed to reach the target
			int numberNeeded = target - currentNumber;

			// Check if the needed number has already been seen (i.e., exists in the map)
			if (numberIndexMap.containsKey(numberNeeded)) {
				// If found, return the indices of the two numbers
				return new int[] { numberIndexMap.get(numberNeeded), currentIndex };
			}

			// Otherwise, store the current number and its index in the map
			numberIndexMap.put(currentNumber, currentIndex);
		}

		// Return null if no valid pair is found (shouldn't happen if input is valid)
		return null;
	}

}
