...
public class Problem_703 {
	...
	/*
	 * 703. Kth Largest Element in a Stream You are part of a university admissions
	 * office and need to keep track of the kth highest test score from applicants
	 * in real-time. This helps to determine cut-off marks for interviews and
	 ...
	 * kthLargest.add(2); // return 7 kthLargest.add(10); // return 7
	 * kthLargest.add(9); // return 7 kthLargest.add(9); // return 8
	 * 
	 * 
	 * Constraints:
	 * 
	 * 0 <= nums.length <= 104 1 <= k <= nums.length + 1 -104 <= nums[i] <= 104 -104
	 * <= val <= 104 At most 104 calls will be made to add.
	 */

	// A min-heap to store the top k largest elements
	PriorityQueue<Integer> minHeap = new PriorityQueue<>();
	// Variable to store k, the number of largest elements we want to keep track of
	int maxLength = 0;

	public static void main(String[] args) {
		Problem_703 kthLargest = new Problem_703(3, new int[] { 4, 5, 8, 2 });
	...
	public int add(int val) {
		minHeap.add(val);
		// If the size exceeds k, remove the smallest elements until size == k
		while (minHeap.size() > maxLength) {
			minHeap.poll();
		}
		return minHeap.peek();
