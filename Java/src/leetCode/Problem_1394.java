package leetCode;

import java.util.HashMap;
import java.util.Map;

public class Problem_1394 {

	// 1394. Find Lucky Integer in an Array
	/*
	 * Given an array of integers arr, a lucky integer is an integer that has a
	 * frequency in the array equal to its value.
	 * 
	 * Return the largest lucky integer in the array. If there is no lucky integer
	 * return -1.
	 * 
	 * 
	 * 
	 * Example 1:
	 * 
	 * Input: arr = [2,2,3,4] Output: 2 Explanation: The only lucky number in the
	 * array is 2 because frequency[2] == 2. Example 2:
	 * 
	 * Input: arr = [1,2,2,3,3,3] Output: 3 Explanation: 1, 2 and 3 are all lucky
	 * numbers, return the largest of them. Example 3:
	 * 
	 * Input: arr = [2,2,2,3,3] Output: -1 Explanation: There are no lucky numbers
	 * in the array.
	 * 
	 * 
	 * Constraints:
	 * 
	 * 1 <= arr.length <= 500 1 <= arr[i] <= 500
	 * 
	 */
	
	public static void main(String args[]) {
		
		int[] test1 = {2,2,3,4};
		System.out.println(findLucky(test1));
	}
	
	public static int findLucky(int[] arr) {
	    // Create a HashMap to count the frequency of each number in the array
	    Map<Integer, Integer> map = new HashMap<Integer, Integer>();
	    
	    for (int i = 0; i < arr.length; i++) {
	        // If the number is already in the map, increment its count
	        if (map.containsKey(arr[i])) {
	            map.put(arr[i], map.get(arr[i]) + 1);
	        } else {
	            // Otherwise, initialize its count to 1
	            map.put(arr[i], 1);
	        }
	    }

	    // Initialize the result variable to -1 (default return value if no lucky number is found)
	    int maxValue = -1;

	    // Iterate through the entries in the map to find lucky numbers
	    for (Map.Entry<Integer, Integer> entry : map.entrySet()) {
	        // A number is "lucky" if its value equals its frequency
	        // Use .equals() to compare Integer values, not ==, because:
	        // == only works reliably for Integer values between -128 and 127 due to Java's Integer caching
	        if (entry.getKey().equals(entry.getValue())) {
	            // Update the maximum lucky number found so far
	            maxValue = Math.max(maxValue, entry.getKey());
	        }
	    }

	    // Return the largest lucky number found, or -1 if none exists
	    return maxValue;
	}

}
