...
public class Problem_3349 {
	 ...
	 * increasing. The subarrays must be adjacent, meaning b = a + k. Return true if
	 * it is possible to find two such subarrays, and false otherwise.
	 * 
	 * 
	 * 
	 * Example 1:
	 ...
	 * increasing. These two subarrays are adjacent, so the result is true. Example
	 * 2:
	 * 
	 ...
	 * Constraints:
	 * 
	 * 2 <= nums.length <= 100 1 < 2 * k <= nums.length -1000 <= nums[i] <= 1000
	 */
	...
	public static boolean hasIncreasingSubarrays(List<Integer> nums, int k) {
		...
		int left2 = k, right2 = 2 * k - 1;

		while (right2 < nums.size()) {
			boolean first = true;
			boolean second = true;
